function [sibling,uncle] = dual_get_relatives(key,variable_tree)
%%
...
if isempty(tail)
    if isfield(branch,'gamma') && length(branch.gamma)>1
...
if isempty(tail_tail)
    ...
    if isfield(branch_branch,'gamma') && length(branch.gamma)>1
...
[sibling,uncle] = dual_get_relatives(key,variable_tree_tail);
end
