function [data,ranges] = nephew_blur(data_ft,bank,ranges,sibling,uncle)
...
if level_counter>0
    ...
    for node = 1:numel(data_ft)
        ...
        [data{node},ranges_node] = ...
            nephew_blur(data_ft{node},bank,node_ranges,sibling,uncle);
...
if nData_dimensions>1
    error('nephew_blur for multiple variables at uncle level not ready yet');
end
...
for uncle_index = 1:nUncle_gammas
    ...
    if isempty(sibling)
        [data{uncle_index},ranges_node] = ...
            firstborn_blur(data_ft{uncle_index},bank,ranges_node);
    elseif sibling.is_firstborn
        [data{uncle_index},ranges_node] = ...
            secondborn_blur(data_ft{uncle_index},bank,ranges_node,sibling);
    else
        [data{uncle_index},ranges_node] = ...
            sibling_blur(data_ft{uncle_index},bank,ranges_node,sibling);
    end
    try
