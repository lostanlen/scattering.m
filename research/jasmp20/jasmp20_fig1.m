...
opts{1}.banks.time.max_scale = 1024;
opts{1}.banks.time.is_chunked = false;
opts{1}.banks.time.wavelet_handle = @morlet_1d;
opts{1}.banks.time.gamma_bounds = [1 round(5.0*nfo)];
...
for wav_name_id = 1:length(wav_names)
    ...
    t = 0:1/sr:N/sr;
    if wav_name_id == 1
        waveform = circshift(waveform, [4096*2, 0]);
    end
    waveform = planck_taper(N, 0.5) .* waveform;
    ...
    U2_yticks = interp1(alphas, 1:length(alphas), 1:10) * resize_factor;
    ...
    column = 1+0*[U{1+2}{1,1}{1}.variable_tree.time{1}.gamma{2}.leaf.metas(U{1+2}{1,1}{1}.ranges{3}(1,1):U{1+2}{1,1}{1}.ranges{3}(3,1)).resolution];
    row = 0+1*[U{1+2}{1,1}{1}.variable_tree.time{1}.gamma{1}.gamma{1}.leaf.metas.resolution];

    resolution_matrix = (row' * column).';
    ...
    S2_concat = cat(2, S2_pos, S2_neg(:, end:-1:1));

    figure(2*wav_name_id-1);
    U1_sc = display_scalogram(U{1+1}{1});
    imagesc(log1p(1e0*U1_sc(:, (1+end/4):(3*end/4))));
    ...
    colormap rev_magma;
    set(gcf(), 'Position', [100+200*wav_name_id, 100, ...
        120*1.618, 120]);
    ...
    for ytick_id = 1:n_yticks
        plot([128, 128+8], [128-U2_yticks(ytick_id), 128-U2_yticks(ytick_id)], ...
            'k', 'LineWidth', 2.0);
    end
    global LineWidthOrder, LineWidthOrder=[3];
    arrow3([129, 0.5], [129, 129+7], 'k/', 1.5, 2);
    global LineWidthOrder, LineWidthOrder=[3];
    arrow3([0.5, 0.5], [256+16.5, 0.5], 'k/', 1.5, 2);
    text( ...
        136, 136, '$\alpha \textrm{ (Hz)}$', ...
        'FontSize', 32.0, 'Interpret', 'latex');
    text( ...
        256+9, 9, '$\beta \textrm{ (c/o)}$', ...
        'FontSize', 32.0, 'Interpret', 'latex');
    xlim([-18, 256+18])
    ylim([-10, 129+30]);
    colormap rev_magma;
    hold off;
    axis off;
    set(gcf(), 'Position', [500+100*wav_name_id, 500, ...
        600, 500]);
