function [Y,initnLambda]=generate_DB_frompath(dataset_path)
...
chunk_paths = dir(dataset_path);
chunk_paths=chunk_paths(~ismember({chunk_paths.name},{'.','..','.DS_Store'}));

%% initialize dataset 
disp('initialize DB')
[stereo_waveform, sample_rate] = audioread_compat([dataset_path '/' chunk_paths(1).name]);
