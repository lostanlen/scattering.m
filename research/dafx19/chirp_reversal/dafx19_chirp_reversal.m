...
% Synthesize chirps.
x = eca_load('path/to/your/input.wav', N);
%x = cat(1, ...
%    repmat(dafx19_synthesize_chirps(N/8), 7, 1), zeros(N/8, 1));

% Setup time-frequency scattering operators.
archs = dafx19_setup(N, Q1, T);
...
imagesc(log1p(orig_scalogram(10:50, 1:(8192*16))./100.0));
axis off;
subplot(212);
scalogram = display_scalogram(U{1+1});
imagesc(log1p(scalogram(10:50, 1:(8192*16))./100.0));
axis off;
colormap rev_magma