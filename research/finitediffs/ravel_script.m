file_path = 'ravel_original_at17640Hz.wav';
...
while iteration < nIterations
    ...
    if reconstruction_opt.is_signal_displayed
        ...
        if mod_iteration==0
            ...
            out_signal = ...
                0.9 * signal * max(abs(target_signal)) / max(abs(signal));
            export_str = ...
                ['ravel_', arch_type, '_it', ...
                 num2str(iteration, '%0.3d'), '.wav'];
            audiowrite(export_str, out_signal, sample_rate);
        end
    end
    toc();
end